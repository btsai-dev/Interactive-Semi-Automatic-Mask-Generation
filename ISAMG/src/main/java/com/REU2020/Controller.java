...
public class Controller {
    ...
    public void initData(Image img, File file){
        ...
        imgFile = file;
        orig = Imgcodecs.imread(Controller.imgFile.getAbsolutePath(), Imgcodecs.IMREAD_GRAYSCALE);
        worker = null;
        resetBlur();
        /**
    ...
    public void saveMask(){
        try{
            ...
            Alert saved = new Alert(Alert.AlertType.INFORMATION,
                    "Something went wrong while saving the mask. Close the window to skip.",
                    ButtonType.OK);
            saved.setHeaderText(null);
            saved.showAndWait();
            e.printStackTrace();
        ...
        try {
            ...
            if (osName.startsWith("Windows")) {
                ...
                if (bitness == 32) {
                    ...
                    in = App.class.getResourceAsStream("/opencv/x86/opencv_java342.dll");
                    fileOut = File.createTempFile("lib", ".dll");
                } else if (bitness == 64) {
                    System.out.println("64 bit detected");
                    in = App.class.getResourceAsStream("/opencv/x64/opencv_java342.dll");
                    fileOut = File.createTempFile("lib", ".dll");
                } else {
                    System.out.println("Unknown bit detected");
                    in = App.class.getResourceAsStream("/opencv/x86/opencv_java342.dll");
                    fileOut = File.createTempFile("lib", ".dll");
                }
            }
